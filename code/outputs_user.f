...
c Grunes addition (added cloud_frac_grid to this list ^)
...
c Grunes additions
c	 &  cloud_frac_grid(nx_max,ny_max)
...
c Define the output variable data block. if Grunes, change this n_vars value
      parameter (n_vars=27)
...
c 22 = tsfc(i,j) = Tsfc(i,j)
      c_var(22) = 'y'

c 23 = qmlt(i,j) = Qm(i,j)
      c_var(23) = 'y'

c 24 = qfrz(i,j) = Qf(i,j)
      c_var(24) = 'y'

c 25 = qcnd(i,j) = Qc(i,j)
      c_var(25) = 'y'

c 26 = qlat(i,j) = Qe(i,j)
      c_var(26) = 'y'

c 27 = qsen(i,j) = Qh(i,j)
      c_var(27) = 'y'
...
c Values averaged over the period.
          vars(i,j,21) = vars(i,j,21) + albedo(i,j) / print_inc
c Grunes: If cloud_frac is something you want to output, fix this! This (^) is a 
c 	placeholder to make the code run. 
c New variables.
            vars(i,j,22) = vars(i,j,22) + Tsfc(i,j) / print_inc
            vars(i,j,23) = vars(i,j,23) + Qm(i,j) / print_inc
            vars(i,j,24) = vars(i,j,24) + Qf(i,j) / print_inc
            vars(i,j,25) = vars(i,j,25) + Qc(i,j) / print_inc
            vars(i,j,26) = vars(i,j,26) + Qe(i,j) / print_inc
            vars(i,j,27) = vars(i,j,27) + Qh(i,j) / print_inc
