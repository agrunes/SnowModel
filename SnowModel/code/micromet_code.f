...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  wind_lapse_rate,prec_grid_sol,pertPrec, pertTair,snowfall_frac)
      ...
      real snowfall_frac     ! parameter for snowfall fraction calculation
     ...
     &  barnes_lg_domain,UTC_flag,wind_lapse_rate,pertPrec,pertTair 

c     real pertPrec     ! perturbation for precipitation
c     real pertTair     ! perturbation for air temperature
...
c   valid observations to be interpolated.
     ...
     &  igrads_metfile,iter,pertPrec,pertTair,irun_data_assim,
     &  icorr_factor_loop)
...
c       print *,'   solving for precipitation'
        print *, 'calling precip, snowfall_frac = ', snowfall_frac
     ...
     &    snowmodel_line_flag,xg_line,yg_line,topo_ref_grid,rh_grid,
c Grunes additions
     &    snowfall_frac)
...
c J.PFLUG      
     ...
     &    barnes_lg_domain,n_stns_used,k_stn,snowmodel_line_flag,
     &    xg_line,yg_line,topo_ref_grid,prec_grid_sol,rh_grid)
c Grunes addition
c	 &	  rh_grid,snowfall_frac)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  snowmodel_line_flag,xg_line,yg_line,topo_ref_grid, rh_grid,
     &  snowfall_frac)
c Grunes addition (rh_grid,snowfall_frac)
...
c Grunes additions
	  ...
	  real temp4
	  real temp5
c end Grunes additions
      print *, 'snowfall_frac = ', snowfall_frac
...
c Begin Grunes additions
c Calculation of precipitation as snow or rain depending
c   snowfall_frac parameter. Note here that,
...
c   snow-water-equivalent per time step.
      Tf = 273.16
c Calculation of rain-snow threshold based on Auer (1974) single-value threshold
c   at +2.0 C. (snowfall_frac = 1)
		if (snowfall_frac.eq.1.0) then
		print *, 'using single-value R-S threshold'
		  do j=1,ny
			do i=1,nx
			  ...
			  sprec(i,j) = snowfall_frac_4 * prec_grid(i,j)
			enddo
		  enddo
...
c   and 1. (snowfall_frac = 2)
		elseif (snowfall_frac.eq.2.0) then
          print *, 'using Dai rain-snow threshold'
		  do j=1,ny
			do i=1,nx
			  Tair_C = Tair_grid(i,j) - Tf
			  temp1 = (tanh(0.7205 * (Tair_C - 1.1662)) - 1.0223)
              snowfall_frac_4 = - 0.482292 *temp1
			...
			enddo
		  enddo
c Jennings, K. et al. (2018): Spatial variation of the rain–snow temperature 
c   threshold across the Northern Hemisphere, Nature Communications,
c   9, 1148 (2018), doi:10.1038/s41467-018-03629-7.
c In this implementation I have clipped values to 0
c   and 1 similar to Glen's code for Dai version.
        elseif (snowfall_frac.eq.3.0) then

          do j=1,ny
            do i=1,nx
              Tair_C = Tair_grid(i,j) - Tf
			  temp1 = exp(-10.04 + 1.41 * Tair_C + 0.09 * rh_grid(i,j))
              p_snow = 1 / (1 + temp1)
c       &        exp(-10.04 + 1.41 * Tair_C + 0.09 * rh_grid(i,j)))
              if (p_snow.lt.0.5) then
                snowfall_frac_4 = 0.0
              else
                snowfall_frac_4 = 1.0
              endif
            ...
            enddo
          enddo
c Calculation of rain-snow threshold at wet bulb temperature
c   threshold of +0.5 C (snowfall_frac = 4). Wet bulb temperature 
c   calculated as a function of air temperature and relative humidity. 
		elseif (snowfall_frac.eq.4.0) then
		print *, 'using T-w rain-snow threshold'
		  do j=1,ny
			do i=1,nx
				Tair_C = Tair_grid(i,j) - Tf
				relhumid = rh_grid(i,j)
				temp1 = Tair_C * ATAN(0.151977 * SQRT(8.313659 + relhumid))
				temp2 = ATAN(Tair_C + relhumid)
				temp3 = -ATAN(relhumid - 1.676331)
				temp4 = 0.00391838 * (relhumid ** 1.5)
				temp5 = ATAN(0.023101 * relhumid)

				t_wetbulb = temp1 + temp2 + temp3 + (temp4 * temp5) - 4.686035


				if (t_wetbulb.lt.0.5) then
c Adjust this single-value threshold here ^
				  snowfall_frac_4 = 1.0
				else
				  snowfall_frac_4 = 0.0
				endif
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  icorr_factor_loop)
      ...
      real pertPrec,pertTair,u1,u2,PI,randErr
...
c Grunes addition of pertTair into this workflow
      if (pertTair.ne.0.0) then
c generate a normal random number using the box-mueller transformation
        u1 = rand()
        u2 = rand()
c pi as according to maximum precision of compiler
...
c constant perturbation scalar
          ...
          print *, 'Running precipitation perturbations model'
...
c ADD A.Grunes
        if (pertTair.ne.0.0) then
c constant perturbation scalar
          print *, 'Running temperature perturbations model'
          print *, 'Tair_orig(k) = ', Tair_orig(k)
          Tair_orig(k) = Tair_orig(k) + pertTair
c         print *, 'Running temperature perturbations model'
          print *, 'pertTair = ', pertTair
          print *, 'Tair_new(k) = ', Tair_orig(k)
c          randErr = (Tair_orig(k)*.15)*sqrt(-2*log(u1))*cos(2*PI*u2)
c          if (irun_data_assim.eq.1.and.icorr_factor_loop.eq.1) then
c            write (11,990) randErr
c          elseif (irun_data_assim.eq.1.and.icorr_factor_loop.eq.2) then
c            read (11,*) randErr
c          endif
c add random error to Temperature
c          Tair_orig(k) = Tair_orig(k) + randErr
        endif
c END A.Grunes
...
c   here and set lapse_rate_user_flag = 1 in the .par file.
      data lapse_rate_user /4.6,5.6,4.9,5.0,4.9,3.7,
     &                      1.0,8.1,7.7,6.8,6.0,5.05/
