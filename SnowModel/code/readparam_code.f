...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  fc_param,t_avg,albedo_flag,pertPrec,pertTair,depth_assim,
     &  snowfall_frac, ro_adjust)
...
c Put parameter names here:
     ...
     &  al_min,al_dec_cold,al_dec_melt,fc_param,t_avg,pertPrec,pertTair,
     &  snowfall_frac, ro_adjust
...
c temperature perturbation
          if (c_param(1:i_param_chars).eq.'pertTair') then
            ipar_count = ipar_count + 1
            cpar_name(ipar_count) = c_param(1:i_param_chars)
            call char2real(pertTair,i_value_chars,c_value,
     &        c_param(1:i_param_chars))
          endif
...
c snowfall_frac 
     ...
     &        c_param(1:i_param_chars))
            if (snowfall_frac.ne.1.0 .and. snowfall_frac.ne.2.0 .and.
     &        snowfall_frac.ne.3.0 .and. snowfall_frac.ne.4.0) then
              print *,'snowfall_frac not 1.0,2.0,3.0 or 4.0'
              stop
            endif
          endif
c ro_adjust
          if (c_param(1:i_param_chars).eq.'ro_adjust') then
            ipar_count = ipar_count + 1
            cpar_name(ipar_count) = c_param(1:i_param_chars)
            call char2real(ro_adjust,i_value_chars,c_value,
     &        c_param(1:i_param_chars))
          endif
